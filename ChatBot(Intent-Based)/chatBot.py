...
"""Hello Devs, this part of the code is not having voice feature, that i made for my internship project submission."""
...
#         #check for chat_log.csv file, if not present then creting it
#         if not os.path.exists("chat_log.csv"):
#             with open("chat_log.csv","w",newline="",encoding='utf-8') as csvfile:
#                 csv_writer = csv.writer(csvfile)
#                 csv_writer.writerow(["User","Bot","Time"])
#             counter += 1
#             user_input = st.text_input("You: ",key=["user_input_{counter}"])
#             if user_input:
#                 #converting the user input into string
#                 user_input_str = str(user_input)
#                 response = chatbot(user_input)
#                 st.text_input("ChatBot:",value=response,height = 120, max_chars=None ,key=["chatbot_response_{counter}"])
#                 #current time
#                 timestamp = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
#                 #saving the inputsin the in the csv file
#                 with open("chat_log.csv","a",newline="",encoding='utf-8') as csvfile:
#                     csv_writer = csv.writer(csvfile)
#                     csv_writer.writerow([user_input_str,response,timestamp])
#                 if response.lower() in ['goodbye', 'bye', 'exit', 'quit']:
#                     st.write("Chatbot: Goodbye! Have a nice day!")
#                     st.stop()
...
#     #about menu
#     elif choice == "About":
#         st.write("The goal of this project is to create a chatbot that can undrestand and respond to user intents.")
#         st.subheader("Project Overview")
#         st.write("This chatbot is based on intents. It is trained on predefined patterns and responses.")
#         st.write("It classifies user intents based on predefined patterns and responses.")
#         st.subheader("Conclusion")
#         st.write("This chatbot can be further improved by training it on more patterns and responses.")
#         st.subheader("Open Source")
#         st.write("This project is open source. You can find the code in the following link, and feel free to ask any of you query and please contribute in it in you way 👍.")
#         st.write('''Future Enhancements that I am thiking of are\n
# - Expand Dataset: Include more intents and responses to handle a wider variety of user inputs.
# - Advanced NLP Techniques: Implement more sophisticated NLP models, such as BERT or GPT, for better understanding and context.
# - Multi-language Support: Add support for multiple languages to cater to a diverse audience.
# - Sentiment Analysis: Integrate sentiment analysis to make the chatbot more responsive to user emotions.
# - Voice Integration: Allow users to interact with the chatbot via voice input for a more interactive experience.
# - Personalization: Incorporate user-specific data to provide personalized responses and recommendations.
# - Continuous Learning: Implement a feedback loop for the chatbot to learn from user interactions and improve over time. ''')
#         st.write("GitHub: [Chatbot](https://github.com/markstone111")
